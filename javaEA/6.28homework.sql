...
alter table tb_emp1 add constraint emp1_dept_fk foreign key(deptId) references tb_dept(id);
...
select fruits.f_name from fruits inner join (select fruits.s_id from fruits where fruits.f_id='a1') e
 on fruits.s_id=e.s_id; 
...
select fruits.*,s.s_name from fruits inner join (select s_name,s_id from suppliers ) s on fruits.s_id=s.s_id;

-- 查询所有价格小于9的水果的信息，查询s_id等于101和103所有的水果的信息，使用UNION连接查询结果
select * from fruits where fruits.f_price<9 union
select *from fruits where fruits.s_id in (101,103);

-- 查询所有价格小于9的水果的信息，查询s_id等于101和103的所有水果的信息，使用UNION ALL连接查询结果
select * from fruits where fruits.f_price<9 union all
select *from fruits where fruits.s_id in (101,103);
...
select concat(s_name , s_city ) as suppliers_title
from suppliers ;
...
select c_name,grade from score s where s.stu_id=(select id from student where name='李四');
...
where(select stu_id from score where c_name='计算机')=(select stu_id from score where c_name='英语'));

-- 从student表和score表中查询出学生的学号，然后合并查询结果
select student.id from student inner join score on student.id=score.stu_id;

-- 查询姓张或者姓王的同学的姓名、院系和考试科目及成绩
select name,department,score.c_name,score.grade from student inner join score
 on student.id=score.stu_id where student.name like '张%' or student.name like '王%';

-- 查询都是湖南的学生的姓名、年龄、院系和考试科目及成绩
select student.name,2020-birth,student.department,score.c_name,score.grade from student
 inner join score on student.id=score.stu_id where student.address like '%湖南%';

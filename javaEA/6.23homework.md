...
select * from fruits where f_price<2.00 or f_price>10.20;
...
select max(e_salary) ,dept_no from employee  group by dept_no;
...
select count(e_no),dept_no from employee group by dept_no;

-- 在employee表中，计算不同类型职工的总工资数。
select sum(e_salary),e_job from employee group by e_job;

-- 在employee表中，计算不同部门的平均工资。
select avg(e_salary),e_job from employee group by e_job;
...
select sum(o_item),o_num from orderitems group by o_num;
...
select * from fruits where f_name = (select max(f_name) from fruits);
